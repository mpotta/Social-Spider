...
accuracy = 0;
x  = zeros(m,1);
finalOutput = [samples(:,numInput+1), samples(:,numInput+2), x, x];
...
for i = 1:m
       % Compute Hidden Layer Node Values
        ...
        finalOutput(i,3) =  outputLOutput(1,1);
        finalOutput(i,4) =  outputLOutput(1,2);
        
        if(finalOutput(i,1) == 0)
            if(finalOutput(i,3) > finalOutput(i,4))
                accuracy = accuracy + 1;
            end;
        else
            if(finalOutput(i,4) > finalOutput(i,3))
                accuracy = accuracy + 1;
            end;
        end;
